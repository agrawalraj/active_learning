...
for n, b, k, strat in itr.product(ns, bs, ks, strategies):
    if 'entropy-multiple-mec' == strat:
        if k is None:
            cmd = f'python3 run_experiments.py -n {n} -b {b} -m 2 -s .1 -i gauss --folder erdos_renyi_final --strategy entropy-dag-collection-multiple-mec'
        else:
            cmd = f'python3 run_experiments.py -n {n} -b {b} -k {k} -m 2 -s .1 -i gauss --folder erdos_renyi_final --strategy entropy-dag-collection-multiple-mec'
    if 'random' == strat:
        if k is None:
